	...
	uint32_t *hashes_done)
{
	...
	if (device_sm[dev_id] >= 750) intensity *= 1.478;
	...
	else intensity *= 0.579;

	intensity = floor(throughput2intensity((uint32_t)intensity) * 4.0) * 0.25;

	uint32_t throughputmax = (uint32_t)((1.0 + (intensity - floor(intensity)))*(1UL << (int)intensity));
...
#else
		CUDA_SAFE_CALL(cudaMalloc(&d_hash2, 4 * 4 * sizeof(uint64_t) * throughputmax));
#endif
